...
export const createReceiptPdfBlob = async (transaction: Transaction) => {
  ...
  const notesExtraHeight = transaction.orderNotes ? 14 : 0;
  const pageHeight = Math.max(205, 178 + transaction.items.length * 12 + notesExtraHeight);
  const doc = new jsPDF({
  ...
  addKeyValue(
    ...
    transaction.orderType === 'Dine in' && transaction.tableNumber
      ? `${transaction.orderType} - Meja ${transaction.tableNumber}`
  ...
  if (transaction.orderNotes) {
    doc.setFont('helvetica', 'normal');
    doc.text('Catatan', 8, y);
    doc.setFont('helvetica', 'bold');
    doc.text(doc.splitTextToSize(transaction.orderNotes, 36), 72, y, { align: 'right' });
    y += 10;
  }
  doc.line(8, y, 72, y);
